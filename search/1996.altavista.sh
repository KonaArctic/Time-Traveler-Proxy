...
test $1 -eq 0 &&
	echo "<b> Documents 1-10  of about `echo $SEARCH_ITEMS | cut -c 1 -``echo $SEARCH_ITEMS | cut -c 2- - | tr '[0-9]' '0' `  matching the query,  best matches first.</b><br></P><dl>" ||
...
for i in `head -n 10 -` ; do
	...
	# I don't know what compact or detailed form looked like. So I'll just guess for now.

	echo "$HTTP_URI" | grep -qe 'fmt=c' - &&
		SEARCH_SNIPPET=''
